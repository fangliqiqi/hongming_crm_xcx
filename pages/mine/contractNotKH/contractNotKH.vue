	...
	<view class="contractNotKH">
		...
		<u-dropdown class="seek-select" menu-icon="arrow-down-fill" menu-icon-size='14' inactive-color='#3A3B3D'
			...
			<u-dropdown-item v-model="value2" title="签约时间" :options="options2">
				<calendar :judge='judge' :usableHeight='usableHeight' :emptyJdge='emptyJdge' :modePrice='modePrice' @send="getSonValue"></calendar>
				<view class="slot-bottom">
					<view class="bottom-empty" @click="empty">清空</view>
					<view class="bottom-confirm" @click="confirm">确定</view>
				</view>
		...
		<view class="contractNotKH_contyent">
			<view class="overhide">
...
<script>
	import calendar from '@/components/calendar/calendar.vue'; //时间选择器
	export default {
		components: {
			calendar
		},
		...
		methods: {
			...
			getcontractList() {
				...
				this.$u.post('/marketMyCompanyContract/queryListPage?' + params).then(resp => {
					console.info('我的合同-非客户')
					console.info(resp.rows)
					...
					if (this.contractList.length == 0) {
						...
						this.contractList = (this.page == 1 ? [] : this.contractList).concat(resp.rows)
					}
				})
			},
			// 跳转到合同详情
			toDetail(e) {
				let item = e;
				uni.navigateTo({
					// url: '/pageKh/hetong/contract/contract?id=' + item.id
					// url: '/pageKh/hetong/contract/contract?id=' + encodeURIComponent(JSON.stringify(item))
