<template>
	<view class="warningTasks">
		<!-- 预警任务 -->
		<view class="seek">
			<view class="seek_box">
				<u-icon name="search" color="#C1C2C7" size="32"></u-icon>
				<input placeholder='请输入搜索内容' class="input" @input="getList" />
			</view>
		</view>
		<view class="content">
			<view class="content_child" v-for="(item,index) in contentList">
				<view class="child_title">
					...
					<view class="title_hint">
						<image src="@/static/image/yujing.png" mode="widthFix" style="width: 10.5px;height: 10.5px;" />
				...
				<view class="child_content">
					...
					<!-- 	<view class="name">
						...
						<view class="name">
							<text class="name_key">执行人：</text>
							<text class="name_price">{{item.createName}}</text>
						</view>
					</view>
	...
	export default {
		...
		methods: {
			getList(event) {
				...
				this.$u.post('/fieldSupervisor/findEarlyWarningMission', {
