<template>
	<view class="myClient">
		...
		<view class="content" v-if="contentList!=0">
			<view class="content_child" v-for="(item,index) in contentList">
				<text class="child_title"  @click="skip(item)">{{item.companyName}}</text>
	...
	<view v-else  class="seek">暂无数据哦</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			onKeyInput(event) {
				...
				this.$u.post('/fieldSupervisor/clientDetails',{blurryKeyword:this.grabble}).then(resp => {
			...
			getList() {
				...
				this.$u.post('/fieldSupervisor/clientDetails?1=1&' + params).then(resp => {
			...
			skip(item){
				this.$store.state.id=item.companyId
				  uni.navigateTo({
								     // url: "/pages/messageBox/messageBox",//跳转页面的路径
									 url: "/pageWqzg/fieldSupervisor/homePage/myClient/orderForm/orderForm?item=" +
      encodeURIComponent(JSON.stringify(item)),
			...
			followUp(item) {
				var item = item
				uni.navigateTo({
					url: "/pageWqzg/fieldSupervisor/homePage/myClient/followUp/followUp?item=" +
						encodeURIComponent(JSON.stringify(item)),
				})
			}
		},
