<template>
	<view class="clientNumber">
		<!-- 客户数 -->
		<view class="clientNumber_select">
			...
			getList() {
				this.$u.post('/boss/clientReport', {
					...
					if (resp == [] || resp.length == 0) {
						...
						this.ec.option.yAxis[0].data = []
					} else {
						this.contentList = resp
						var nameArray = []
						var dataArray = []
						this.contentList.forEach((item, index) => {
							dataArray.push(item.nubmer)
							nameArray.push(item.city)
						})
