<template>
	<view class="abnormalOrders">
		<view class="child" v-for="(item,index) in dataArray">
			<text
				class="name">{{item.map.companyName||''}},订单编号:{{item.map.orderNumber||''}},{{item.map.moneyType||''}}{{item.map.amountOwed||''}}元未交</text>
			<view class="bottom">
				<text class="time">{{item.map.time||''}}</text>
				<text class='bottomName'>{{item.map.createName||''}}</text>
			</view>
		</view>
...
<script>
	export default {
		...
		onLoad(option) {
			...
			// 	if(item.moneyType==10){
			// 		item.moneyType='尾款'
			// 	}else if(item.moneyType==20){
			// 		item.moneyType='回款'
			// 	}
			// })
			// this.dataArray = item
		...
		methods: {
			getList() {
				...
				this.$u.get('/messageReceiving/findAbnormalOrderMessage?1=1&' + params).then(resp => {
